...
export class HelperStringService implements IHelperStringService {
    /** Генерирует референс с таймстемпом и случайной частью */
    ...
    /** Генерирует случайную строку по заданным опциям */
    ...
    /** Маскирует часть строки звездочками */
    ...
    /** Проверяет валидность email-адреса по простому шаблону */
    checkEmail(email: string): boolean {
        const regex = new RegExp(/\S+@\S+\.\S+/);
        return regex.test(email);
    }

    /** Проверяет сложность пароля по требованиям */
    checkPasswordStrength(
        password: string,
        options?: IHelperStringPasswordOptions
    ): boolean {
        const rule = options?.rule ?? 'strict';
    ...
    /** Форматирует число как валюту по локали */
    formatCurrency(
        num: number,
