...
export function PaginationFilterNinEnumPipe<T>(
    field: string,
    defaultValue: T,
    defaultEnum: Record<string, unknown>,
    ...
    class MixinPaginationFilterInEnumPipe implements PipeTransform {
        ...
        async transform(value: string): Promise<Record<string, unknown>> {
            if (options?.raw) {
        ...
        addToRequestInstance(
            ...
            const filters = this.request.__pagination?.filters
                ? { ...this.request.__pagination.filters, [field]: value }
                : { [field]: value };
